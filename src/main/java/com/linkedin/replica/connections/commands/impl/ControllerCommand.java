...
public class ControllerCommand extends Command {
    ...
    @Override
    public Object execute() throws SecurityException, IllegalArgumentException, NoSuchMethodException, IllegalAccessException {
        ...
        try{
            ...
            throw new ConnectionsException(ex.getTargetException().getMessage());
        }
        return null;
    }
    ...
    public static void setMaxThreadCount(Object val){
        if(val == null || ! ((JsonPrimitive) val).isNumber())
            throw new ConnectionsException(String.format("Invalid parameters : %s. expected an integer representing maximum number of threads", val ));
    ...
    public static void setMaxDBConnectionsCount(Object val){
        if(val == null || !((JsonPrimitive) val).isNumber())
            throw new ConnectionsException(String.format("Invalid parameters : %s. expected an integer representing maximum number of DB connections", val ));
    ...
    public static void setErrorReportingLevel(Object val){
        if(val == null || ! ((JsonPrimitive) val).isNumber())
            throw new ConnectionsException(String.format("Invalid parameters : %s. expected an integer representing logging level.", val ));
    ...
    private static void validate(JsonObject obj, String... args){
        for(String arg : args){
            if(obj.get(arg) == null )
                throw new ConnectionsException(String.format("Invalid parameters : %s. expected : %s", obj.keySet().toString(), Arrays.toString(args)));
        }
    }
